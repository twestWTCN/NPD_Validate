...
for i = 1:length(data.label)
    for j = 1:length(data.label)
        ...
        npdspctrm{1,1}(i,j,:) = 2.4.*f13(:,10);
        npdspctrm{1,2}(i,j,:) = 2.4.*f13(:,12); % Backward
        npdspctrm{1,3}(i,j,:) = 2.4.*f13(:,11); % Forward
        npdcrcv(i,j,:) = t13(:,3);
        nscohspctrm(i,j,:) = f13(:,4);
        
        [f13,t13,c13]=sp2_R2a_pc1(x',y',z',data.fsample,2^npdord);
        npdspctrmZ{1,1}(i,j,:) = 2.4.*f13(:,10);
        npdspctrmZ{1,2}(i,j,:) = 2.4.*f13(:,12); % Backward
        npdspctrmZ{1,3}(i,j,:) = 2.4.*f13(:,11); % Forward
        npdcrcvZ(i,j,:) = t13(:,3);
        
        [f13,t13,c13]=sp2_R2a_pc1(x',y',w',data.fsample,2^npdord);
        npdspctrmW{1,1}(i,j,:) = 2.4.*f13(:,10);
        npdspctrmW{1,2}(i,j,:) = 2.4.*f13(:,12); % Backward
        npdspctrmW{1,3}(i,j,:) = 2.4.*f13(:,11); % Forward
        npdcrcvW(i,j,:) = t13(:,3);
