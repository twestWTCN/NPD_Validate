...
%         y = (y-mean(y))./std(y);
    ...
    npd(ncov) = max(npdspctrm{1,2}(1,2,Hz>42 & Hz<62))- max(npdspctrm{1,3}(1,2,Hz>42 & Hz<62));
...
%     plotNPD(Hz,npdspctrm,data,cmapn(3,:),plotfig,linestyle)
    ...
    %% GRANGER
    [Hz granger grangerft] = computeGranger(freq,cmapn(2,:),Nsig,plotfig,linestyle,1);
