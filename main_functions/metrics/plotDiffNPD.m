function plotDiffNPD(Hz,npdspctrm,data,cmap,plotfig,linestyle,confindflag)
if nargin<7
    confindflag = 0;
end
%% PLOT NPD
...
for i = 1:length(data.label)
    for j = 1:length(data.label)
        ...
        if i>j
            fy = squeeze(npdspctrm{1,2}(j,i,:))-squeeze(npdspctrm{1,2}(i,j,:));
            cy = squeeze(npdspctrm{2,2}(j,i,:));
        elseif j>i
            fy = squeeze(npdspctrm{1,2}(j,i,:))-squeeze(npdspctrm{1,2}(i,j,:));
            cy = squeeze(npdspctrm{2,2}(j,i,:));
        elseif j==i
            fy = squeeze(npdspctrm{1,1}(i,j,:))-squeeze(npdspctrm{1,1}(i,j,:));
