...
if mvflag == 1
    %Multivariate
    [grangerft,igrangerft] = NPG_freq(freq,[],bstrp,bstraptype);
end
    ...
    for j = 1:length(freq.label)
        if mvflag == 0 % Pairwise
            if i == j
                ...
                [grangerft,igrangerft] = NPG_freq(freq,[i j],bstrp,bstraptype); % Recompute the estimate using pairs of data (rest of data unseen)
            end
            if i<j
                ip = 1; jp = 2;
            else
...
% if plotfig
%     plotNPD(grangerft.freq,granger,freq,cmap,1,linestyle,bstrp)
% %     plotDiffNPD(grangerft.freq,granger,freq,cmap,1,linestyle,bstrp)
% end
